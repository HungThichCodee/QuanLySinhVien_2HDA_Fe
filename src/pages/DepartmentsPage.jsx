  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editItem) {
        ...
        setToast({ message: 'Tạo mới thành công', type: 'success' })
      }
      setModalOpen(false)
    } catch (err) {
    ...
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={function () { setModalOpen(false) }} title={editItem ? 'Sửa Khoa' : 'Thêm Khoa'}>
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Tên Khoa</label>
          ...
          <div className="mb-6">
            ...
            <textarea value={form.description} onChange={function (e) { setForm({ ...form, description: e.target.value }) }}
              className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" rows="3"></textarea>
          </div>
