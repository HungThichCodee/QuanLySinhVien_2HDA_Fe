...
import * as gradeService from '../services/grades.service.js'
...
function GradesPage() {
  ...
  async function handleUpdate(gradeId, field, value) {
    try {
      ...
      setToast({ message: 'Cap nhat diem thanh cong', type: 'success' })
    } catch (err) { setToast({ message: err.message, type: 'error' }) }
  }
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-800 font-display mb-6">{isStudent ? 'Bang diem ca nhan' : 'Quan ly Diem'}</h1>

      {(isTeacher || isAdmin) && (
        <div className="mb-6"><label className="text-sm font-medium text-gray-700 mr-3">Chon lop hoc phan:</label>
          <select value={selectedCC} onChange={function (e) { loadCCGrades(e.target.value) }} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary">
            <option value="">-- Chon --</option>{courseClasses.map(function (cc) { return <option key={cc._id} value={cc._id}>{cc.subject?.name} - {cc.semester?.name}</option> })}
          </select>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-card overflow-hidden"><table className="w-full"><thead><tr className="border-b border-gray-200 bg-gray-50"><th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">#</th>
        {isStudent && <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Mon hoc</th>}
        {!isStudent && <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Sinh vien</th>}
        ...
        {grades.length === 0 && <tr><td colSpan="6" className="px-5 py-8 text-center text-gray-400 text-sm">Khong co du lieu</td></tr>}
      </tbody></table></div>
    </div>
  )
}
