...
function TeachersPage() {
  ...
  async function handleSubmit(e) { e.preventDefault(); try { await createItem(form); setToast({ message: 'Tạo GV thành công', type: 'success' }); setModalOpen(false); fetchData() } catch (err) { setToast({ message: err.message, type: 'error' }) } }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6"><h1 className="text-2xl font-bold text-gray-800 font-display">Quản lý Giáo viên</h1><button onClick={function () { setModalOpen(true); setForm({ username: '', password: '', email: '', fullName: '', phone: '', department: '' }) }} className="bg-primary text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-primary-dark transition-colors">+ Thêm GV</button></div>
      ...
      <Modal isOpen={modalOpen} onClose={function () { setModalOpen(false) }} title="Thêm Giáo viên">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4"><label className="block text-sm font-medium text-gray-700 mb-1.5">Mật khẩu</label><input type="text" value={form.password} onChange={function (e) { setForm({ ...form, password: e.target.value }) }} placeholder="VD: GiaoVien123@" className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required /></div>
